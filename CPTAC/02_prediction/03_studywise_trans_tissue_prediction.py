...
def get_studywise_data(study):
    ...
    transcriptomic = cptac_transcriptomics_study[cptac_transcriptomics_study['Study'] == study].drop(columns=['Study']) 
    other_studies = cptac_transcriptomics_study[cptac_transcriptomics_study['Study'] != study].drop(columns=['Study'])
    
    # Select only those proteins with both proteomic and transcriptomic data available for at least 60% 
    # samples in each study
    proteomics = proteomics.reindex(genes_to_predict, axis=1)
    transcriptomics = transcriptomic.reindex(genes_as_features, axis=1)
    other_studies = other_studies.reindex(genes_as_features, axis=1)
    
    return(transcriptomics, other_studies, proteomics)


def run_parallelized_baseline_pipeline(study, feature_set, desired_filename): 
    all_folds_results = []
    print("Running " + study)
    transcriptomics_study, other_studies, proteomics_study = get_studywise_data(study)
    
    assert proteomics_study.shape[1] == 4046, "Number of proteins to predictis not as expected for the test study"
    assert transcriptomics_study.shape[1] == 12580, "Number of input features to be used is not as expected for the test study"
    assert other_studies.shape[1] == 12580, "Number of input features to be used is not as expected for other studies"
        
    kf = KFold(n_splits=5, random_state=None, shuffle=False)
    for fold, (train, test) in enumerate(kf.split(transcriptomics_study)):
        X_train = pd.concat([transcriptomics_study.iloc[train], other_studies])
        X_test = transcriptomics_study.iloc[test]
        y_train = proteomics_study.reindex(X_train.index)
        y_test = proteomics_study.reindex(X_test.index)
        all_folds_results.append(proteomics_study.columns.to_series().parallel_apply(lambda p: \
...
fallback_method_prefix = "tm_results_fallback_"
corum_file_prefix = 'tm_results_corum_'
pi400_file_prefix = "tm_results_pi400_"
pi800_file_prefix = "tm_results_pi800_"
ppi400_file_prefix = "tm_results_ppi400_"
ppi800_file_prefix = "tm_results_ppi800_"
vae100_file_prefix = "tm_results_vae100_"
vae500_file_prefix = "tm_results_vae500_"
vae100_pi400_file_prefix = "tm_results_vae100_pi400_"
vae500_pi400_file_prefix = "tm_results_vae500_pi400_"
vae1000_file_prefix = "tm_results_vae1000_"
vae1000_pi400_file_prefix = "tm_results_vae1000_pi400_"
all_transcriptomes_file_prefix = "tm_results_all_transcriptomes_"
